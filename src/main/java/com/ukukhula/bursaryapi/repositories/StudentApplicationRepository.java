...
public class StudentApplicationRepository {
  ...
  private final RowMapper<StudentApplication> studentRowMapper = (
    resultSet,
    rowNumber
  ) -> {
  ...
  public StudentApplication findByStudentID(int studentID) {
    ...
    StudentApplication students = jdbcTemplate.queryForObject(
      SQL,
      studentRowMapper,
      studentID
    );
  ...
  public University findStudentUniversity(int studentId) {
    String query =
      "SELECT * FROM Universities u LEFT JOIN Students s ON u.UniversityID = s.UniversityID WHERE s.StudentID = ?";
    return jdbcTemplate.queryForObject(query, UniversityRowMapper, studentId);
  }

  public String getEthnicity(int studentid) {
    String query =
      "SELECT Ethnicity FROM Ethnicities e " +
      "LEFT JOIN Students s " +
      "ON s.EthnicityID=e.EthnicityID WHERE s.StudentID=?";

    String ethnicity = jdbcTemplate.queryForObject(
      query,
      String.class,
      studentid
    );
  ...
  public Integer updateStudentsApplicationColumnValue(
    ...
    final String SQL =
      "UPDATE StudentApplications SET " +
      columnName +
      " = ? WHERE StudentID = ?";
  ...
  public int deleteApplication(int applicationID) {
    String sql =
      "DELETE FROM StudentApplications WHERE StudentApplicationID = ?";
    return jdbcTemplate.update(sql, applicationID);
  }

  public int studentApplication_ActiveStudent(
    StudentApplicationDetails_ActiveStudent studentApplicationDetails_ActiveStudent
  ) {
    try {
      int bursaryDetailsId = new BursaryDetailsRepository(jdbcTemplate)
        .getBursaryDetailsByYear(
          studentApplicationDetails_ActiveStudent.getYear()
        )
        .getBursaryDetailsId();
          ...
          new HashMap<String, Object>() {
            {
              put(
                "StudentID",
                studentApplicationDetails_ActiveStudent.getStudentID()
              );
              put(
                "Motivation",
                studentApplicationDetails_ActiveStudent.getMotivation()
              );
              put(
                "BursaryAmount",
                studentApplicationDetails_ActiveStudent.getBursaryAmount()
              );
              put(
                "StatusID",
                studentApplicationDetails_ActiveStudent.getStatusID()
              );
              put(
                "Reviewer_UserID",
                studentApplicationDetails_ActiveStudent.getReviewer_UserID()
              );
              put(
                "ReviewerComment",
                studentApplicationDetails_ActiveStudent.getReviewerComment()
              );
              put("Date", studentApplicationDetails_ActiveStudent.getDate());
              put(
                "UniversityStaffID",
                studentApplicationDetails_ActiveStudent.getUniversityStaffID()
              );
  ...
  public int studentApplication_NewStudent(
    StudentApplicationDetails_NewStudent studentApplicationDetails_NewStudent
  ) {
    try {
      int bursaryDetailsId = new BursaryDetailsRepository(jdbcTemplate)
      ...
      int universityId = new UniversityRepository(jdbcTemplate)
        .getUniversityIdByName(
          studentApplicationDetails_NewStudent.getUniversityName()
        )
        .getUniversityId();

      int departmentId = new DepartmentRepository(jdbcTemplate)
        .getDepartmentIdByName(
          studentApplicationDetails_NewStudent.getDepartmentName()
        )
        .getDepartmentId();

      int ethnicityID = new EthnicityRepository(jdbcTemplate)
        .getEthnicityIdByName(
          studentApplicationDetails_NewStudent.getEthnicity()
        )
        .getId();
          ...
          new HashMap<String, Object>() {
            {
              put(
                "FirstName",
                studentApplicationDetails_NewStudent.getFirstName()
              );
              put(
                "LastName",
                studentApplicationDetails_NewStudent.getLastName()
              );

              put(
                "PhoneNumber",
                studentApplicationDetails_NewStudent.getPhoneNumber()
              );
              ...
              put(
                "IDNumber",
                studentApplicationDetails_NewStudent.getIdNumber()
              );
              ...
              put(
                "Motivation",
                studentApplicationDetails_NewStudent.getMotivation()
              );
              put(
                "BursaryAmount",
                studentApplicationDetails_NewStudent.getBursaryAmount()
              );
              put(
                "StatusID",
                studentApplicationDetails_NewStudent.getStatusID()
              );
              put(
                "Reviewer_UserID",
                studentApplicationDetails_NewStudent.getReviewer_UserID()
              );
              put(
                "ReviewerComment",
                studentApplicationDetails_NewStudent.getReviewerComment()
              );
              put("Date", studentApplicationDetails_NewStudent.getDate());
              put(
                "UniversityStaffID",
                studentApplicationDetails_NewStudent.getUniversityStaffID()
              );
              put("BursaryDetailsID", bursaryDetailsId);
            }
          }
      ...
      Map<String, Object> out = simpleJdbcCall
        .withProcedureName("uspAddStudentApplication_NewStudent")
        .execute(in);
